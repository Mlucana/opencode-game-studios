...
| Symbol | Type | Range | Description |
|---|---|---|---|
| `S_k` | int | 6–42 ticks (rango completo: mín 18−24/2, máx 30+24/2; con `J = 14`: 11–37) | Ticks entre cierre de `Golpe k` y apertura de `Golpe k+1` (el Telegrafiado interno) |
...
**Output range:** cada `S_k` entero en `[S_base − J/2, S_base + J/2]`; `J < 14` → FAIL_LOAD; `S_base − J/2 < 11` → FAIL_LOAD (suelo conjunto provisoria: recuperación-techo 3 + margen-suelo 8; OQ3 autorizada a moverla tras barrido Deck). Sin clamp runtime — el sorteo es al autorar/cargar el duelo.
...
- [ ] **D2 antimash_J (Logic, TR-jefe-010)** — GIVEN un combo con `S_base = 24`, WHEN `J ∈ {13, 14}`, THEN `J = 13` falla al cargar y `J = 14` pasa; AND el barrido `J ∈ {2, 4, 6, 9, 12}` documenta en el log que todos ≤13 fallan (regresión del dato medido 93/93); AND WHEN `S_base = 18`, `J = 24` THEN falla por suelo conjunto (`18 − 12 = 6 < 11`) aunque cada knob esté en rango — prueba de que la conjunta no es redundante (espejo de D3-agregado y D13(b) Combate). No se asevera tasa de mash aquí (eso es C12b de Combate con harness reactivo) — aquí solo las guardas de datos.
...
- [ ] **D5 tutorial (Logic, TR-jefe-010)** — GIVEN todos los patrones simples del Vigilante + la separación inter-patrones de sus combos `N = 3`, WHEN se validan, THEN `separacion_inter ≥ 13 + 9 + margen` con los knobs vivos (no literales) y `N ≤ 3` y cero especiales; un simple con separación 33 y piso 34 falla. `S_k` intra se excluye aquí (lo cubre D2).
- [ ] **D5b ancha_vigilante (Logic, TR-jefe-010)** — GIVEN los combos `N = 3` del Vigilante, WHEN se validan, THEN `S_base ∈ [28, 30]` y `J = 14`.
- [ ] **D6 derivacion_vida (Logic, TR-jefe-010)** — GIVEN tríada con `ciclos_objetivo` (4/5/6) y `punish_dano_pct` (25%/20%/16.7%), WHEN se declara `vida_max_angel`, THEN `vida_max_angel / dano_golpe_castigo == ciclos_objetivo` exacto (`dano = vida × punish% × 1.0`; tolerancia ±1e-6 por 100/6 no representable; tras clamp E12) — p. ej. Cosmos `dano` 40 → `vida` 200 (5×40); Humanidad `dano` 30 → `vida` 120; Cercanía `dano` 50 → `vida` 300. Fuera → FAIL_LOAD. Por arriba (vida mayor) el duelo grindea sin pedir más aciertos/ciclo; por abajo rompe el suelo de ciclos.
...
- [ ] **X1 contrato_placeholders (Integration)** — GIVEN build limpia con el sistema 2 en 5ª pasada (declarado, no cableado), WHEN se listan los símbolos que este GDD asume del #2, THEN `accion_especial_completada` y `castigo_conectado` constan como **declarados-no-cableados** (cero referencias de código, solo mención documental) y `restantes(T)` se evalúa en forma inclusiva (`ventana + 1 − T`, borde `ventana − gracia`, nunca literal); AND el test falla si aparece el literal 114 o el conteo exclusivo en cualquier fixture nuevo.
