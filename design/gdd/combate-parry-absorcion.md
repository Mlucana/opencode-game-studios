...
2. **Ventana de parry**: al presionar el botón, el jugador entra en estado Parry
   ...
   **Autoridad única del tiempo — cuatro reglas normativas** (revalidadas contra
   Godot 4.7 por `godot-specialist` en la re-review de 2026-08-01):
    ...
    1. **Incrementar por tick, nunca acumular delta — condicional al mecanismo adoptado.** Bajo el mecanismo decidido (Pattern-A de Feedback R1/R12, APPROVED: freeze = `SceneTree.paused` diegético; `Engine.time_scale` descartado, ver nota abajo), escalar el tiempo cambia el valor de `delta` que reciben los consumidores **sin** cambiar la cadencia de `_physics_process` (`physics_ticks_per_second`): por eso un contador entero (`ticks += 1`) es inmune al hitstop y un acumulador `elapsed += delta` **no lo es**. **Esta afirmación es condicional al mecanismo, no una propiedad del motor**: bajo `Engine.time_scale` global ocurriría lo inverso —la cadencia por segundo real escalaría y el `delta` por tick permanecería fijo en `1/60`. Con Pattern-A se lee con "delta escalado" = `DiegeticTick` congelado-por-construcción, y el contador entero sigue siendo la única forma legal. Bajo pausa total la física pausable corre 0 ticks en freeze: el contador inmune es el de pared (WallTick), no el diegético.
   2. **El hitstop escala la simulación diegética, nunca la capa de HUD/UI.** Por decisión de diseño (revisión 7ª pasada) el hitstop es **pausa total del subárbol diegético (0%)**, no escala al 4%: durante
   un hitstop, el mundo (jugador, ángel, VFX, cámara) queda congelado
   mientras la capa de HUD/UI sigue avanzando a velocidad normal (ver Feedback de Impacto, Regla 1 — transient de entrada + congelación 0%). Es obligatorio
      ...
      > sobre imagen congelada (0%-pausa) es exactamente lo contrario de lo que el resto de
   ...
   3. **Las duraciones de hitstop se canonicalizan en ticks, igual que todo lo demás.**
      ...
      tick es la fuente de verdad. Lo que sigue vigente de la formulación anterior:
       ...
       escalado — bajo pausa-0% el tiempo diegético no avanza durante el freeze por construcción (esa es la intención); leerlos a velocidad 0.04 es la semántica antigua, retirada.
...
| Calidad de timing | `calidad_timing` | float **escalonado** | conjunto cerrado de 7 valores (tabla abajo) | `clamp(1 − (Δ − 0.5) / umbral_precision, 0, 1)`, con **`Δ = |t_press − t_golpe|` en ticks enteros (valor absoluto)** entre el tick de detección del input y el tick de inicio del **`Golpe`** — **no de toda ventana parable**, ver la acotación abajo. Un press anterior al inicio usa `|Δ|` (p. ej. 12 ticks antes → `calidad 0.0`, nunca Justo). Mide cercanía al instante exacto; NO determina éxito/fallo (eso ya es binario, Regla 3) |
...
**Ejemplo**: `dano_base = 10`, pulsación detectada **2 ticks** después del inicio del
`Golpe` → `calidad_timing = 1 − (2 − 0.5)/5 = 0.7` →
...
**Output Range:** 100–179 con los valores de lanzamiento (100+54+25, techo vinculante de R10a; 185 con `bono_vida_por_absorcion` en su techo 20; rango declarado de knobs: 80–235, 110–235 en suelo efectivo R5; 9-ángeles hasta ~305 — ver R5). El techo **ya no queda abierto
...
| Golpes para morir | `golpes_para_morir_base` | int | 3–6 (actual: **4**; validación al cargar: dentro de rango 3–6, ver glosario FAIL_LOAD en C16 — fuera de rango, incluido `0` = división por cero, falla) | `Golpe`s no parados que matan a un jugador **a Vida base**, sin absorciones ni reliquias. Es el **presupuesto de error del duelo**. **Vida lógica float** (display redondeado, comparación exacta `±1e-6`) |
...
> hace la división **una sola vez** (en float, nunca `int/int` — en GDScript trunca y rompe E13/D15 en silencio) y con los valores de lanzamiento da un entero
...
> `dano_golpe_enemigo` la Vida resultante queda **por debajo de 0**, se **clampa a 0**; el exceso no se
> transfiere, no se acumula y no produce efecto alguno. Un residuo de valor absoluto
...
| **R6** | `parry_window_ticks / (parry_window_ticks + recuperacion_whiff) ≤ 0.65`, equivalente exacto en enteros `parry_window_ticks × 35 ≤ recuperacion_whiff × 65` (la forma `≤ 1.857 ×` redondea y falla en el borde 13/7) | 13 / 22 = **59%** ✓ | El cierre del mash — el hallazgo más grave de la revisión anterior. **Aplica también a los valores *efectivos tras reliquias*, no solo a los de lanzamiento** (ver nota R6 abajo) |
...
> ```
>
> con `tasa_acierto_objetivo = 0.72`, **la tasa medida en el prototipo** (Feel/prototipo, no un
...
> parries que falla (banda de tolerancia ±0.02; re-medir si cambian `parry_window`, `umbral_precision`, shake (`T2`/`S_max`) o cadencia). Un `Golpe` no parado cuesta exactamente **1 unidad** por definición.
>
> **Semántica:** la banda por ventana se evalúa en **esperanza**; la cláusula agregada Σ en **peor caso** (ceil).
>
...
> `2.24` ✗ fuera. El techo robusto de la banda cae en **H ≤ 66** para esa tríada (67–71: zona de re-medida obligatoria bajo tolerancia ±0.02).
>
...
esas cuatro: **no existe la categoría "efecto no clasificado"** (ver glosario FAIL_LOAD en C16). Verificado además por
...
- **`bono_vida_por_absorcion` es el knob más delicado del sistema.** Rango seguro
  ...
  respetar su rango: debe verificarse R5 junto a `vida_base`** — con `vida_base = 100`
  solo 15–18 pasan (19 → 57%, 20 → 60%, ambos > 55%); 20 exige `vida_base ≥ 110` (19 exige ≥104; 110 cubre ambos).
...
|---|---|---|---|
| Hitstop (parry exitoso) | **5 ticks de reloj real** (0.0833s), pausa total del subárbol diegético 0% (ver Regla 2, reglas normativas 2 y 3) | Congela el mundo; validado en prototipo como contribuyente clave al peso. **No congela la capa de HUD/UI.** 5 ticks, no 0.08s: 0.08 × 60 = 4.8 no es entero y contradiría el contador autoritativo | Sí (`hitstop_parry`, **3–6 ticks** — techo recortado de 8 por R8; ver Tuning Knobs) |
...
- [ ] **C1** — DADO un enemigo activo, CUANDO comienza un ciclo, ENTONCES el estado avanza en orden Telegrafiado → Golpe → Enfriamiento → (repite), sin poder saltar fases ni repetir Golpe sin pasar por Enfriamiento (intra-combo: Golpe→Golpe legal sin Enfriamiento — ver Regla 9).
...
- [ ] **C3** — DADO un parry activo, CUANDO una **ventana parable** comienza dentro de la ventana activa (caso b) O el jugador presiona durante una **ventana parable** activa (caso a), ENTONCES el intento se resuelve como éxito **una única vez**, sin doble resolución si ambas condiciones se cumplen. **Verificable en las 4 combinaciones**: los dos tipos de ventana parable (`Golpe`, Ventana Especial) × los dos casos (a) y (b). **Partido: C3a `Golpe` ejecutable hoy; C3b Ventana Especial bloqueado — mismo fixture que C19.** *(Requantificado el 2026-08-04, enmienda A: la formulación anterior nombraba solo `Golpe` y por tanto el AC pasaba en verde mientras la Ventana Especial era imparable — el AC estaba calibrado al mismo subconjunto que la regla que verificaba.)*
- [ ] **C4** *(instrumentado)* — DADO un parry exitoso **resuelto contra un Golpe simple o contra el golpe final/de interrupción de un combo (intermedios excluidos — ver C9/D4)**, CUANDO se resuelve, ENTONCES se aplican las cuatro consecuencias en el mismo **tick** de simulación (convención fotograma=tick), verificado por orden de señales con `signal_order_spy`: daño de Postura, Gracia otorgada, hitstop disparado, enemigo en Repliegue. *(Acotación añadida en la 2ª pasada de `/design-review` del sistema 2: para un parry resuelto contra una **Ventana Especial** se aplican solo 2 de las 4 — ver la excepción de la Regla 4 y el AC **C5a** del sistema 2. Sin esta acotación, C4 y la Core Rule 5 del sistema 2 eran mutuamente incumplibles.)*
...
- [ ] **C7a** — DADO cualquier estado del jugador, CUANDO presiona parry (éxito, whiff o fallo), ENTONCES el intento no cuesta: ΔVida == 0 exacto; ΔGracia ≥ 0 (la absorción en éxito es ganancia — ver Regla 4/C20 —, nunca coste), verificado por contadores.
- [ ] **C7b** — Ningún otro recurso del conjunto cerrado {Vida, Gracia} cambia con un parry; un recurso nuevo debe añadirse al conjunto (falla review, no este AC).
- [ ] **C8a** — DADO 0 ángeles absorbidos y 0 bonos de reliquia, CUANDO se calcula `vida_maxima`, ENTONCES el resultado es exactamente 100.
- [ ] **C8b** — DADO 0 ángeles absorbidos y 0 bonos, CUANDO se absorbe 1 ángel, ENTONCES `vida_maxima` aumenta exactamente en `bono_vida_por_absorcion` (100→118 con valores de lanzamiento).
...
- [ ] **C13a** *(instrumentado)* — DADO un parry exitoso **no-Justo** que dispara hitstop, CUANDO se mide su duración en **reloj real**, ENTONCES el contador de ticks es **exactamente 5** (vía contador, sin tolerancia) y la duración wall-clock es `5×16.666ms ±4ms` a 60Hz (sin rama 40Hz dedicada — removida 8ª pasada; ver P1 por modos).
- [ ] **C13b** *(instrumentado)* — DADO un Parry Justo (`calidad_timing = 1.0` → 7 ticks, `= 0.9` → 6 ticks), ENTONCES el contador es exactamente el total correspondiente y `hitstop_parry + bono ≤ 8` (**R8**). Verifica las reglas normativas 1 y 3 de la Regla 2 a la vez: si la implementación acumulase `delta` en vez de contar ticks, la duración se dispararía bajo la escala de tiempo diegética; si interpretase los ticks como tiempo escalado, duraría ~2 segundos.
...
- [ ] **C16** *(instrumentado — requiere inyectar datos de patrón malformados; no ejecutable por un tester de caja negra sin herramienta de carga; **bloqueado** hasta que exista el loader de patrones del sistema 20. Glosario: "falla la validación / falla ruidosamente" = exit no-cero + línea ERROR + no arranca duelo — aplica a C16/D13/C26/C24/D8b. RUNTIME_DROP (runtime) = evento descartado + línea WARN + contador — aplica a payloads corruptos en duelo)* — DADO un patrón de ataque encadenado instanciado por el sistema 20, CUANDO se valida su longitud, ENTONCES `3 ≤ N ≤ 5` (Regla 9). Un patrón con N=2 o N≥6 debe **fallar la validación de datos al cargar**, no degradarse en silencio. Es la restricción que este GDD impone al sistema 20; sin este AC, violarla no rompería nada visible hasta que fallase R7 o el cue de audio.
...
> ACs C19–C21 **y sus suscriptores (C3b, C10b, C25b, V5–V7, D1b; C22/C23 exigen además el loader del sistema 20)** requieren una **Ventana Especial** activa, que hoy solo puede
...
> primer sprint; **sin él están bloqueados**. Dos niveles: trigger-suficiente (C3b/C10b/C25b/V5–V7/D1b) vs trigger+loader (C22/C23/D13). Tercer nivel (V7-segunda-mitad): evento de completación del sistema 2.
>
...
- [ ] **C22-completación** *(instrumentado; **bloqueado** hasta que exista el disparador de depuración de Ventana Especial — ver el fixture compartido arriba)* — DADO una **Ventana Especial** que vence **sin ser parada** (cierre: ver **C21**), CUANDO la `Acción Especial` **se completa**, ENTONCES **la Vida del jugador sigue siendo numéricamente idéntica** —el sistema 2 declara que ese estado nunca la reduce— y la **magnitud que la habilidad declara perturbar** cambia exactamente en la cantidad declarada, cuya conversión a unidades de `dano_golpe_enemigo` cae dentro de la banda de **R9a**. **Verificable en los dos bordes**: severidad equivalente 1.0 y 2.0.
...
- [ ] **C25** *(instrumentado)* — DADO un parry exitoso **resuelto contra un `Golpe`**, CUANDO `calidad_timing ≥ umbral_parry_justo` (0.9 con los valores de lanzamiento), ENTONCES se clasifica como **Parry Justo** y disparan **las tres** señales a la vez y sobre el mismo predicado (salvo coincidencia con 8, donde rige precedencia — variante tímbrica del cierre, ver V4): la variante de esquirla hacia `#FFF8E7`, la capa de audio adicional (evento 4) y el bono de hitstop. CUANDO `calidad_timing < umbral_parry_justo`, ENTONCES **ninguna de las tres** dispara. El bono de hitstop se reparte por escalón: `calidad_timing = 1.0` → **+2 ticks**, `= 0.9` → **+1 tick**, por debajo → **+0**; en todos los casos se cumple **R8** (`hitstop_parry + bono ≤ 8`: 5+2 = 7 ✓).

   **Y DADO un parry exitoso resuelto contra una Ventana Especial**, ENTONCES **ninguna de las tres dispara nunca**, sea cual sea el timing: `calidad_timing` no se calcula para ese caso (ver la acotación de la Fórmula 1) y el hitstop es el base, sin bono. **Partido: C25a `Golpe` ejecutable hoy *(instrumentado)*; C25b Ventana Especial bloqueado — mismo fixture que C19.** *(Acotación añadida el 2026-08-04, changeset 2 ítem 3, corrigiendo el cuantificador con el que este AC se escribió en el ítem 1.)*
...
- [ ] **D1** *(instrumentado)* — DADO `dano_base = 10`, `bono_precision = 0.4` y `umbral_precision = 5 ticks` (valores de lanzamiento), CUANDO se resuelve un parry exitoso **contra un `Golpe`** con distancia medida `Δ` en ticks enteros, ENTONCES `calidad_timing` y `postura_dano` toman **exactamente** estos pares y ningún otro (tolerancia `±1e-6` en ambas coordenadas, ver **D14**):
...
- [ ] **D1b** *(instrumentado; mismo fixture que C19)* — DADO un parry exitoso resuelto contra una **Ventana Especial**, ENTONCES **no** se calcula `calidad_timing`, **no** se clasifica como Parry Justo y **no** se aplica bono de hitstop (ver **C25b**).
...
- [ ] **D8b** *(instrumentado)* — DADO `golpes_para_morir_base` fuera de 3–6, no-entero o 0, CUANDO se valida al cargar, ENTONCES falla (ver glosario FAIL_LOAD en C16); bordes 2-falla/3-pasa/6-pasa/7-falla/0-falla/4.5-falla.
...
- [ ] **D13** *(**bloqueado** hasta que exista el loader de patrones del sistema 20; ver glosario FAIL_LOAD en C16)* — DADO un patrón del sistema 20 con `interrumpible_por_parry = true`, CUANDO se valida al cargar, ENTONCES se comprueban **las dos cláusulas de R9a por separado**: **(a)** la severidad declarada de cada `Acción Especial` cumple `1.0 ≤ severidad_accion_especial ≤ 2.0` — verificable en los cuatro bordes: **0.99 falla, 1.0 pasa, 2.0 pasa, 2.01 falla**; y **(b)** la suma de severidades de todas las Ventanas Especiales que ese patrón puede emitir en un duelo cumple `Σ severidad ≤ golpes_para_morir_base − 1` (= **3** con los valores de lanzamiento). Un patrón que viole cualquiera de las dos **falla la validación de datos al cargar**, no se degrada en silencio. **Y (c)**: el patrón debe **declarar la magnitud que la habilidad perturba y su equivalencia** (`magnitud_perturbada`, `equivalencia_dano`) en unidades de `dano_golpe_enemigo` (regla de conversión de R9a, con fidelidad: equivalencia declarada vs recomputada ±1e-6 a tasa pineada); una `Acción Especial` interrumpible **sin equivalencia declarada falla también** — no existe la categoría "coste no cuantificado", que es exactamente lo que era *"el jefe completa su habilidad"* antes del changeset 2.
...
- [ ] **D14** *(instrumentado)* — DADO cualquier secuencia de parries exitosos **resueltos contra un `Golpe`**, CUANDO se registran todos los valores de `calidad_timing` producidos, ENTONCES **(a)** todo valor pertenece al conjunto cerrado de **7 escalones** de D1 con tolerancia `±1e-6` — **ningún valor intermedio es alcanzable** — y cada par registrado (`calidad_timing`, `postura_dano`) coincide con una fila de **D1** dentro de `±1e-6` en ambas coordenadas; y **(b)** la función es **monótona no creciente en `Δ`**: nunca un `Δ` mayor produce una calidad mayor.
...
- [ ] **D15** *(instrumentado; **diferido** hasta el catálogo del sistema 9 — mismo tipo de bloqueo que D10)* — DADO cualquier configuración de reliquias equipadas, CUANDO se calcula `golpes_sobrevividos = ⌈vida_maxima / dano_golpe_enemigo⌉` **para los cuatro valores de `angeles_absorbidos` (0, 1, 2, 3)**, ENTONCES en **todos** ellos el resultado excede como máximo en **1** al de la misma cuenta de absorciones **sin ninguna reliquia** (invariante **R10a**). El techo absoluto es **8 golpes** (con valores de lanzamiento; sin absoluto universal — en otras regiones rige `base+1` puro).

  **El cálculo usa `dano_golpe_enemigo` efectivo tras reliquias** (`dano_efectivo = dano_golpe_enemigo × (1 − reduccion)` sobre Vida — dueño del cómputo: sistema 9 post-carga; este GDD solo valida —; resto por clamp/épsilon), no el de lanzamiento: una reliquia de reducción de daño recibido entra por esta vía y se cuenta aquí. Bordes verificables con los valores de lanzamiento: una reliquia de **+25 de Vida pasa** y una de **+26 falla**; una de **−20% de daño recibido pasa** y una de **−25% falla** (suficiencia launch-scoped vb=100/g=4; en general, **C26** manda por configuración).
...
- [ ] **E8** *(instrumentado)* — DADO que el Golpe del enemigo conecta sin haber sido parado, CUANDO el enemigo transiciona a Enfriamiento, ENTONCES la duración de ese Enfriamiento es idéntica a la del Enfriamiento base del ciclo ±1 tick (sin Repliegue ni extensión; el parry exitoso va a Repliegue 0.7s por D4, no a Enfriamiento).
- [ ] **E9** — DADO 3 ángeles con decisiones "rechazar, absorber, absorber" en ese orden, CUANDO se calcula `vida_maxima`, ENTONCES el resultado es idéntico al de la secuencia "absorber, rechazar, absorber" con las mismas 2 absorciones netas (`100 + 2*18 = 136` en ambos casos, con valores de lanzamiento — E9 verifica orden, no magnitud; derivar el esperado de `bono_vida_por_absorcion`, no codificar 136) — la independencia de orden declarada en Edge Cases se verifica explícitamente, no solo se asume.
...
- [ ] **P1** — DADO un build en **hardware real de Steam Deck** (no PC) y un **disparador de depuración exclusivo de QA que fuerce determinísticamente la coincidencia de los eventos visuales 8 (combo completo) + 11 (punish hit)**, CUANDO el jugador completa 3 duelos consecutivos sin pausas de carga entre ellos — uno por tríada (Humanidad, Cosmos, Cercanía a Dios, en ese orden) — y el peor caso se fuerza **entre 3 y 5 veces** a lo largo de la sesión (N y seed registrados), ENTONCES pasan **tres métricas por separado**: (a) percentil 99 sobre la sesión completa ≤16.6ms en dock / ≤25ms en batería; (b) **percentil 99 calculado solo sobre la ventana de ±5 frames alrededor de cada activación forzada del disparador** ≤16.6ms dock / ≤25ms batería; (c) máximo en ventana, informativo (no gatea). La prueba se ejecuta además **dos veces: en modo dock y en modo portátil/batería** (perfil de refresco registrado en el run, sin rama 40Hz dedicada — removida 8ª pasada), y ambas deben pasar por separado. Toda medición de frame time es **wall-clock de render**, no derivada de ticks de simulación.
...
| ¿Cuánta gracia exactamente concede un parry (`gracia_base`)? | GDD de Sistema de Gracia (sistema 5) | Al autorar sistema 5 | ✅ Cerrada 2026-09-04: `= 1.0` (F-G1 Gracia, registry v12); este GDD mantiene modulador 0.5× y ejemplo |
...
| ¿Suena también el golpe sordo del evento 5 cuando un combo se rompe a mitad (evento 9)? Mecánicamente es el mismo suceso físico, pero el evento 9 solo especifica el cristal disolviéndose | Feedback Sonoro del Parry (sistema 16) | Al autorar sistema 16 | **RESUELTA (espejo 4+8, 7ª pasada): el 9 nunca lleva el thud del 5** — ver Regla 6 de Feedback ("el 9 nunca lleva el thud del 5"); el thud significa daño y el 9 no hace daño |
...
| **¿Qué mecanismo concreto implementa la pausa diegética sin tocar el HUD?** `Engine.time_scale` **queda descartado** — es global y Godot 4.7 no permite eximir subárboles. Decidido (8ª pasada): Pattern-A — freeze = `SceneTree.paused` del subárbol diegético (0%), HUD/UI en `PROCESS_MODE_ALWAYS`; contadores en ticks de pared (WallTick); `Timer`/`Tween`/`AnimationPlayer` pausables congelan gratis (rampa con curva pineada y modo de física verificado); el uniform `TIME` de shader SIGUE corriendo en pausa — inyectar uniform de instancia; audio diegético por bus/duck en ALWAYS (ver Feedback R1); orden `process_physics_priority` + latch con sello (ver Feedback R12). Incluye también el stacking de hitstop entre Parry Justo (+1–2 ticks) y cierre de combo. **Ampliaciones de la 3ª pasada** (`godot-specialist`): (e) el autoload de tiempo debe correr con un `process_physics_priority` **menor** que todo nodo de gameplay que consuma su delta escalado en el mismo tick — Godot no garantiza orden de árbol una vez hay prioridades, y equivocarse produce un retardo de 1 tick en el delta escalado justo en los ticks de borde (inicio/fin de hitstop), que es donde importa; (f) el **audio diegético** entra ahora explícitamente en el bucket escalado (Regla 2, regla normativa 2), así que es un consumidor más a resolver; (g) con el suelo de `hitstop_parry` en 3 ticks, fijar `Tween`/`AnimationPlayer` en modo de física **deja de ser opcional** — ver "Sobre R8" | `godot-gdscript-specialist` / `/create-architecture` | **Antes del primer sprint** — es prerrequisito de implementación, no de diseño | Abierta, pero **reformulada y acotada en la re-review**: la Regla 2 fija ahora cuatro reglas normativas verificables (ACs C13/C14) y el documento ya no nombra ninguna API de motor. Lo que queda es puramente arquitectura. `godot-specialist` verificó que la formulación anterior (Regla 2.2 + `Engine.time_scale` global) era **mutuamente contradictoria e inconstruible** |
