...
4. **Firma destructiva (hold, no doble diálogo).** Toda transición que destruye run o suspensión —Comenzar con SUS vigente, Abandonar (duelo y Hub), Reset, Restaurar .bak, Migración— exige **firma Tipo-A**: mantener 1 s (`firma_hold_ms`, default 1000 ms) sobre el botón destructivo + listado que nombra la pérdida (coro_siguiente, nº+ids de reliquias, gracia_actual, corrupcion_actual) y lo conservado (N llaves, M fragmentos). Flanco fresco obligatorio (soltar+pulsar; un input heredado jamás firma). Foco inicial siempre en la opción segura. Soltar antes del hold, cancelar o kill a mitad = SUS byte-idéntica (nada se borra antes del commit). La invariante "exactamente 2 pasos" queda derogada. Alcance: Firma = Tipo-A hold **salvo Decisión #5** (carve-out explícito: Decisión usa commit irrevocable single-press + flanco fresco, sin hold — el hold protege destrucción de run/SUS; aquí castigaría continuidad, P4).
...
| Decisión (enrutada #5) | Post-reliquia | Commit irrevocable single-press → Hub (sin hold, ver #5) | Jamás `pre_eleccion` como punto de suspensión (R5 Guardado) | Lo fija #5 |
...
| `MENU_DECISION_TITLE` | "El coro espera tu mano" (propiedad #5, enrutado #15; fija `/localize`) |
| `MENU_DECISION_TOMAR_LABEL` | "Tomar {poder}" (propiedad #5, enrutado #15; fija `/localize`) |
| `MENU_DECISION_TOMAR_DESC` | "Tomas {poder}: +{vida} vida máx, +{poso} poso" (propiedad #5, enrutado #15; fija `/localize`) |
| `MENU_DECISION_DEJAR_LABEL` | "Dejar ir" (propiedad #5, enrutado #15; fija `/localize`) |
| `MENU_DECISION_DEJAR_DESC` | "Dejas ir {poder}: −{purga} purga" (propiedad #5, enrutado #15; fija `/localize`) |
| `MENU_DECISION_LEDGER` | "gracia {gracia} · corrupción {corrupcion} · poso {poso}" (propiedad #5, enrutado #15; fija `/localize`) |
| `MENU_DECISION_IRREVERSIBLE` | "Lo hecho, hecho está" (propiedad #5, enrutado #15; fija `/localize`) |
| `MENU_DECISION_QUIT_LINE` | "Si sales ahora, esta elección queda sin guardar" (propiedad #5, enrutado #15; fija `/localize`) |

> **Back-link Decisión [2c-06] cerrado 2026-09-07:** las claves `MENU_DECISION_*` son propiedad de Gracia (#5) y enrutadas por este flujo (#15 transita victoria → reliquias → decisión → hub con commit single-press sin hold, carve-out a Firma Tipo-A R4). Formato owned Menú, copy owned `/localize` + writer; ≤120 chars post-interpolación (ver `decision-gracia.md` DEC-18).
