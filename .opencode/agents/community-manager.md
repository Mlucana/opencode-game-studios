...
permission:
  ...
  websearch: deny
---
> Ported from `.claude/agents/community-manager.md` (Claude Code Game Studios) for OpenCode.
> Original model tier: `haiku` — the model is inherited from your OpenCode session (no `model:` pinned for portability). Pin one in the frontmatter above if you want a fixed model.
