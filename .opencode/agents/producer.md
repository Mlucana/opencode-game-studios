...
steps: 30
---
> Ported from `.claude/agents/producer.md` (Claude Code Game Studios) for OpenCode.
> Original model tier: `opus` — the model is inherited from your OpenCode session (no `model:` pinned for portability). Pin one in the frontmatter above if you want a fixed model.
> Original skill hints `[sprint-plan, scope-check, estimate, milestone-review]` — load via the `skill` tool when relevant.
> OpenCode mapping: `AskUserQuestion` → `question` tool · `Task` → `task` tool (invoke peers with @name) · `TodoWrite` → `todowrite` tool.
> Original `memory: user` — keep persistent notes in `production/session-state/active.md` instead.
