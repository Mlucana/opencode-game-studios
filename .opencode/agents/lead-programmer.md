...
permission:
  ...
  websearch: deny
---
> Ported from `.claude/agents/lead-programmer.md` (Claude Code Game Studios) for OpenCode.
> Original model tier: `sonnet` — the model is inherited from your OpenCode session (no `model:` pinned for portability). Pin one in the frontmatter above if you want a fixed model.
> Original skill hints `[code-review, architecture-decision, tech-debt]` — load via the `skill` tool when relevant.
> OpenCode mapping: `AskUserQuestion` → `question` tool · `Task` → `task` tool (invoke peers with @name) · `TodoWrite` → `todowrite` tool.
> Original `memory: project` — keep persistent notes in `production/session-state/active.md` instead.
