---
description: "Validates completeness and consistency of the project architecture against all GDDs. Builds a traceability matrix mapping every GDD technical requirement to ADRs, identifies cov..."
agent: build
---

# /architecture-review (OpenCode command - ported from CCGS skill `architecture-review`)

> Original skill: `.claude/skills/architecture-review/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/architecture-review $ARGUMENTS`
> Original argument hint: `[focus: full | coverage | consistency | engine | single-gdd path/to/gdd.md]`
