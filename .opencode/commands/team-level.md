...
# /team-level (OpenCode command - ported from CCGS skill `team-level`)

> Original skill: `.claude/skills/team-level/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/team-level $ARGUMENTS`
> Original argument hint: `[level name or area to design] [--review full|lean|solo]`
