---
description: "Fast sprint status check. Reads the current sprint plan, scans story files for status, and produces a concise progress snapshot with burndown assessment and emerging risks. Run ..."
agent: build
---

# /sprint-status (OpenCode command - ported from CCGS skill `sprint-status`)

> Original skill: `.claude/skills/sprint-status/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/sprint-status $ARGUMENTS`
> Original argument hint: `[sprint-number or blank for current]`
