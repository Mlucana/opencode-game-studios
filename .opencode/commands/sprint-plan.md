---
description: "Generates a new sprint plan or updates an existing one based on the current milestone, completed work, and available capacity. Pulls context from production documents and design..."
agent: build
---

# /sprint-plan (OpenCode command - ported from CCGS skill `sprint-plan`)

> Original skill: `.claude/skills/sprint-plan/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/sprint-plan $ARGUMENTS`
> Original argument hint: `[new|update|status] [--review full|lean|solo]`
