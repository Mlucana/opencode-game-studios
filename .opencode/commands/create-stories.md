---
description: "Break a single epic into implementable story files. Reads the epic, its GDD, governing ADRs, and control manifest. Each story embeds its GDD requirement TR-ID, ADR guidance, acc..."
agent: build
---

# /create-stories (OpenCode command - ported from CCGS skill `create-stories`)

> Original skill: `.claude/skills/create-stories/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/create-stories $ARGUMENTS`
> Original argument hint: `[epic-slug | epic-path] [--review full|lean|solo]`
