---
description: "Quality review of test files and manual evidence documents. Goes beyond existence checks — evaluates assertion coverage, edge case handling, naming conventions, and evidence com..."
agent: build
---

# /test-evidence-review (OpenCode command - ported from CCGS skill `test-evidence-review`)

> Original skill: `.claude/skills/test-evidence-review/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/test-evidence-review $ARGUMENTS`
> Original argument hint: `[story-path | sprint | system-name]`
