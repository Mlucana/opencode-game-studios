---
description: "Full localization pipeline: scan for hardcoded strings, extract and manage string tables, validate translations, generate translator briefings, run cultural/sensitivity review, ..."
agent: build
---

# /localize (OpenCode command - ported from CCGS skill `localize`)

> Original skill: `.claude/skills/localize/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/localize $ARGUMENTS`
> Original argument hint: `[scan|extract|validate|status|brief|cultural-review|vo-pipeline|rtl-check|freeze|qa]`
