---
description: "Configure the project's game engine and version. Pins the engine in CLAUDE.md, detects knowledge gaps, and populates engine reference docs via WebSearch when the version is beyo..."
agent: build
---

# /setup-engine (OpenCode command - ported from CCGS skill `setup-engine`)

> Original skill: `.claude/skills/setup-engine/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/setup-engine $ARGUMENTS`
> Original argument hint: `[engine] | [engine version] | refresh | upgrade [old-version] [new-version] | no args for guided selection`
