---
description: "Prepare a day-one patch for a game launch. Scopes, prioritises, implements, and QA-gates a focused patch addressing known issues discovered after gold master but before or immed..."
agent: build
---

# /day-one-patch (OpenCode command - ported from CCGS skill `day-one-patch`)

> Original skill: `.claude/skills/day-one-patch/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/day-one-patch $ARGUMENTS`
> Original argument hint: `[scope: known-bugs | cert-feedback | all]`
