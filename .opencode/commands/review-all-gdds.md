---
description: "Holistic cross-GDD consistency and game design review. Reads all system GDDs simultaneously and checks for contradictions between them, stale references, ownership conflicts, fo..."
agent: build
---

# /review-all-gdds (OpenCode command - ported from CCGS skill `review-all-gdds`)

> Original skill: `.claude/skills/review-all-gdds/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/review-all-gdds $ARGUMENTS`
> Original argument hint: `[focus: full | consistency | design-theory | since-last-review]`
