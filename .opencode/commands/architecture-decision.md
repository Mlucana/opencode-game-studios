---
description: "Creates an Architecture Decision Record (ADR) documenting a significant technical decision, its context, alternatives considered, and consequences. Every major technical choice ..."
agent: build
---

# /architecture-decision (OpenCode command - ported from CCGS skill `architecture-decision`)

> Original skill: `.claude/skills/architecture-decision/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/architecture-decision $ARGUMENTS`
> Original argument hint: `[title] [--review full|lean|solo]`
