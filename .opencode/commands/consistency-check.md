---
description: "Scan all GDDs against the entity registry to detect cross-document inconsistencies: same entity with different stats, same item with different values, same formula with differen..."
agent: build
---

# /consistency-check (OpenCode command - ported from CCGS skill `consistency-check`)

> Original skill: `.claude/skills/consistency-check/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/consistency-check $ARGUMENTS`
> Original argument hint: `[full | since-last-review | entity:<name> | item:<name>]`
