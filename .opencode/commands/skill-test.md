...
# /skill-test (OpenCode command - ported from CCGS skill `skill-test`)

> Original skill: `.claude/skills/skill-test/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/skill-test $ARGUMENTS`
> Original argument hint: `static [skill-name | all] | spec [skill-name] | category [skill-name | all] | audit`
