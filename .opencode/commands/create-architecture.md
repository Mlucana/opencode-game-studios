---
description: "Guided, section-by-section authoring of the master architecture document for the game. Reads all GDDs, the systems index, existing ADRs, and the engine reference library to prod..."
agent: build
---

# /create-architecture (OpenCode command - ported from CCGS skill `create-architecture`)

> Original skill: `.claude/skills/create-architecture/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/create-architecture $ARGUMENTS`
> Original argument hint: `[focus-area: full | layers | data-flow | api-boundaries | adr-audit] [--review full|lean|solo]`
