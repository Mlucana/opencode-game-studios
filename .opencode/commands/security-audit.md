---
description: "Audit the game for security vulnerabilities: save tampering, cheat vectors, network exploits, data exposure, and input validation gaps. Produces a prioritised security report wi..."
agent: build
---

# /security-audit (OpenCode command - ported from CCGS skill `security-audit`)

> Original skill: `.claude/skills/security-audit/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/security-audit $ARGUMENTS`
> Original argument hint: `[full | network | save | input | quick]`
