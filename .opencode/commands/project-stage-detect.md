---
description: "Automatically analyze project state, detect stage, identify gaps, and recommend next steps based on existing artifacts. Use when user asks 'where are we in development', 'what s..."
agent: build
---

# /project-stage-detect (OpenCode command - ported from CCGS skill `project-stage-detect`)

> Original skill: `.claude/skills/project-stage-detect/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/project-stage-detect $ARGUMENTS`
> Original argument hint: `[optional: role filter like 'programmer' or 'designer']`
