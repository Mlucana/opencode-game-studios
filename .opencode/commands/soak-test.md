---
description: "Generate a soak test protocol for extended play sessions. Defines what to observe, measure, and log during long play sessions to surface slow leaks, fatigue effects, and edge ca..."
agent: build
---

# /soak-test (OpenCode command - ported from CCGS skill `soak-test`)

> Original skill: `.claude/skills/soak-test/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/soak-test $ARGUMENTS`
> Original argument hint: `[duration: 30m | 1h | 2h | 4h] [focus: memory | stability | balance | all]`
