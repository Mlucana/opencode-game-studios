---
description: "Scaffold the test framework and CI/CD pipeline for the project's engine. Creates the tests/ directory structure, engine-specific test runner configuration, and GitHub Actions wo..."
agent: build
---

# /test-setup (OpenCode command - ported from CCGS skill `test-setup`)

> Original skill: `.claude/skills/test-setup/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/test-setup $ARGUMENTS`
> Original argument hint: `[force]`
