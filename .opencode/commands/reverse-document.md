...
# /reverse-document (OpenCode command - ported from CCGS skill `reverse-document`)

> Original skill: `.claude/skills/reverse-document/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/reverse-document $ARGUMENTS`
> Original argument hint: `<type> <path> (e.g., 'design src/gameplay/combat' or 'architecture src/core')`
