---
description: "After architecture is complete, produces a flat actionable rules sheet for programmers — what you must do, what you must never do, per system and per layer. Extracted from all A..."
agent: build
---

# /create-control-manifest (OpenCode command - ported from CCGS skill `create-control-manifest`)

> Original skill: `.claude/skills/create-control-manifest/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/create-control-manifest $ARGUMENTS`
> Original argument hint: `[update — regenerate from current ADRs]`
