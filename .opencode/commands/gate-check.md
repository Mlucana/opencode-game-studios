---
description: "Validate readiness to advance between development phases. Produces a PASS/CONCERNS/FAIL verdict with specific blockers and required artifacts. Use when user says 'are we ready t..."
agent: build
---

# /gate-check (OpenCode command - ported from CCGS skill `gate-check`)

> Original skill: `.claude/skills/gate-check/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/gate-check $ARGUMENTS`
> Original argument hint: `[target-phase: systems-design | technical-setup | pre-production | production | polish | release] [--review full|lean|solo]`
> OpenCode mapping: `AskUserQuestion` → `question` tool · `Task` → `task` tool (subagents via @name) · `TodoWrite` → `todowrite` tool.
> If `$ARGUMENTS` is empty and the workflow below requires an argument, show the usage line and ask for it with the `question` tool instead of guessing.
