---
description: "Run the critical path smoke test gate before QA hand-off. Executes the automated test suite, verifies core functionality, and produces a PASS/FAIL report. Run after a sprint's s..."
agent: build
---

# /smoke-check (OpenCode command - ported from CCGS skill `smoke-check`)

> Original skill: `.claude/skills/smoke-check/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/smoke-check $ARGUMENTS`
> Original argument hint: `[sprint | quick | --platform pc|console|mobile|all]`
