---
description: "Concept prototype — validate the core idea is worth designing before writing GDDs. Run right after /brainstorm and /setup-engine. Routes to HTML, Engine, or Paper path based on ..."
agent: build
---

# /prototype (OpenCode command - ported from CCGS skill `prototype`)

> Original skill: `.claude/skills/prototype/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/prototype $ARGUMENTS`
> Original argument hint: `[concept-description] [--path html|engine|paper] [--review full|lean|solo] [--spike]`
