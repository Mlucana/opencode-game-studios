---
description: "Guided, section-by-section GDD authoring for a single game system. Gathers context from existing docs, walks through each required section collaboratively, cross-references depe..."
agent: build
---

# /design-system (OpenCode command - ported from CCGS skill `design-system`)

> Original skill: `.claude/skills/design-system/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/design-system $ARGUMENTS`
> Original argument hint: `<system-name> [--review full|lean|solo]`
