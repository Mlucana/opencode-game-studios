---
description: "Map test coverage to GDD critical paths, identify fixed bugs without regression tests, flag coverage drift from new features, and maintain tests/regression-suite.md. Run after i..."
agent: build
---

# /regression-suite (OpenCode command - ported from CCGS skill `regression-suite`)

> Original skill: `.claude/skills/regression-suite/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/regression-suite $ARGUMENTS`
> Original argument hint: `[update | audit | report]`
