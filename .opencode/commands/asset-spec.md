---
description: "Generate per-asset visual specifications and AI generation prompts from GDDs, level docs, or character profiles. Produces structured spec files and updates the master asset mani..."
agent: build
---

# /asset-spec (OpenCode command - ported from CCGS skill `asset-spec`)

> Original skill: `.claude/skills/asset-spec/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/asset-spec $ARGUMENTS`
> Original argument hint: `[system:<name> | level:<name> | character:<name>] [--review full|lean|solo]`
