---
description: "Detect non-deterministic (flaky) tests by reading CI run logs or test result history. Aggregates pass rates per test, identifies intermittent failures, recommends quarantine or ..."
agent: build
---

# /test-flakiness (OpenCode command - ported from CCGS skill `test-flakiness`)

> Original skill: `.claude/skills/test-flakiness/SKILL.md` — OpenCode also auto-discovers it via the `skill` tool.
> Usage: `/test-flakiness $ARGUMENTS`
> Original argument hint: `[ci-log-path | scan | registry]`
